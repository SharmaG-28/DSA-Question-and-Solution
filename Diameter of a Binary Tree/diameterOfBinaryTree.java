...
class Solution {
    ...
    Result calculateDiameter(Node node) {
        if (node == null)
        ...
        int currentDiameter = Math.max(rootDiameter, Math.max(leftResult.diameter, rightResult.diameter));
        int currentHeight = 1 + Math.max(leftResult.height, rightResult.height);

        return new Result(currentDiameter, currentHeight);
    }
}
